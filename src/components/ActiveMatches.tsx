      ...
      <div className="flex border-b border-white/10 pb-4 mb-5 items-center justify-between">
        <div>
          <h3 className="font-sans font-bold text-xl text-slate-100 flex items-center gap-2">
            <Swords className="w-5 h-5 text-indigo-400" /> Semua Meja Pertandingan
          </h3>
          <p className="text-xs text-slate-300 mt-1 font-sans">
            Daftar seluruh meja pertandingan dari babak awal hingga saat ini. {isAdmin ? 'Masukan hasil laga di babak aktif untuk memperbarui poin.' : 'Pantau hasil tanding secara langsung.'}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {rounds.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-8 text-center text-slate-400">
            ...
            <p className="text-sm">Belum ada pairing untuk turnamen ini.</p>
          ...
          rounds.map((round) => {
            ...
            return (
              <div key={round} className="space-y-4">
                <div className="flex border-b border-white/5 pb-2 mb-3 items-center justify-between">
                  <h4 className="font-sans font-bold text-lg text-slate-200 flex items-center gap-2">
                    Babak {round}
                    {isRoundActive && (
                ...
                {roundMatches.map((match) => {
                  ...
                  return (
                    <div
                      key={match.id}
                      className={`bg-white/5 border rounded-2xl p-4 transition-all flex flex-col gap-4 shadow-sm ${
                        isRoundActive ? 'border-indigo-500/30 hover:bg-white/10' : 'border-white/5 opacity-80'
                      }`}
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-xs font-mono border-b border-white/5 pb-2">
                        <span className={isRoundActive ? 'text-indigo-300 font-bold' : 'text-slate-400 font-bold'}>
                          Papan #{match.tableNumber}
                        </span>
                      ...
                      {/* Organizer Input Action Dashboard (ONLY FOR ACTIVE ROUND) */}
